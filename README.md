...
Трейдеру нужно отслеживать сотни монет по нескольким условиям одновременно
(аномальный объём, изменение цены в %, всплеск волатильности, фандинг, отток общего интереса).
Руками это делать невозможно. Скринер забирает данные с биржи, считает метрики **на стороне БД**
и отдаёт отсортированную витрину + сохраняет воспроизводимые «прогоны» скрининга.

**Главная ценность для рекрутёра** - не свечи, а:
- идемпотентный ETL-пайплайн (батч-бэкфилл + WS-стрим, `ON CONFLICT`, авто-реконнект);
...
Все метрики считаются **в БД**, а не в Python. Примеры - в [db/queries/](db/queries/).
...
- Бэкфилл истории запускается автоматически при старте ingestion-сервиса.
